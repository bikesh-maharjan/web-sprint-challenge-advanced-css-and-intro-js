...
function get20s(data) {
  let art = [];
  // converst string to ineger - 2000

  for (let i = 0; i < data.length; i++) {
    const years = data[i].years; // "1850 - 1900"
    const parseYears = years.split(" - "); // ['1850', '1900']
    const startYear = parseInt(parseYears[0]); // converts string to integer - 1850
    const endYear = parseInt(parseYears[1]); // converst string to ineger - 1900
...
const newArtist = {
  ...
  bio:
    "Lorem ipsum dolor sit amet consectetur adipisicing elit. Velit, culpa odio consectetur quaerat, numquam nostrum eius repudiandae nulla aliquam et voluptatibus assumenda architecto aut accusamus, facere eum quam consequuntur quae ",
};
...
// arr[0] = { id: 0, name: 'artist name' , years: '', paintings: 101}
function lotsOfArt(arr) {
  const hundredPainting = [];
  for (let i = 0; i < arr.length; i++) {
    if (arr[i].paintings > 100) {
      hundredPainting.push(arr[i]);
    }
  }
  //console.log(hundredPainting.length);
  return hundredPainting;
}
